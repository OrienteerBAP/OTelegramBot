...
public class OTelegramBotWidget extends AbstractModeAwareWidget<ODocument> {
	...
	public OTelegramBotWidget(String id, final IModel<ODocument> model, IModel<ODocument> widgetDocumentModel) {
		...
		structureTable = new OrienteerStructureTable<ODocument, OProperty>("docsTable", model, propertiesModel){
			@Override
			protected Component getValueComponent(String id, IModel<OProperty> rowModel) {
				return new AbstractModeMetaPanel<ODocument, DisplayMode, OProperty,ODocument >(id, getModeModel(), OTelegramBotWidget.this.getModel(), rowModel) {
